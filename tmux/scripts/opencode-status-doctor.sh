...
printf '%-10s %-7s %-7s %-7s %-5s %-5s %s\n' WINDOW STATE PANE PID ALIVE STALE NAME
...
echo
echo "Tip: STALE = state recorded but agent gone. Clear with:"
echo '  tmux set-option -uw @opencode-state   # (per window)'